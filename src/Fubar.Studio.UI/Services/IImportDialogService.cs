...
public interface IImportDialogService
{
    ...
    /// <summary>The same dialog for a Postman collection. It used to write straight into the workspace
    /// with no preview, so re-importing silently overwrote whatever had been edited since.</summary>
    Task<ImportDialogResult?> ShowPostmanAsync(string workspaceRoot);

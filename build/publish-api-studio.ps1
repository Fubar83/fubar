...
<#
.SYNOPSIS
    Publishes Fubar API Studio as self-contained binaries for one or more runtimes and packages each
...
.EXAMPLE
    ./build/publish-api-studio.ps1              # all default runtimes
    ./build/publish-api-studio.ps1 -Runtimes osx-arm64  # just one
    ./build/publish-api-studio.ps1 -Version 0.1.0-beta.1
#>
param(
...
$project     = Join-Path $repoRoot 'src/Fubar.Studio.UI/Fubar.Studio.UI.csproj'
$exeName     = 'FubarAPIStudio'    # matches <AssemblyName>
$displayName = 'Fubar API Studio'
$bundleId    = 'dev.fubar.apistudio'
...
foreach ($rid in $Runtimes) {
    ...
    if ($rid -like 'osx-*') {
        ...
        $icns = Join-Path $repoRoot 'src/Fubar.Studio.UI/Assets/fubar.icns'
